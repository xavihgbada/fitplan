...
- `send-checkin-reminders.js` — triggered daily by `vercel.json`'s cron (`0 9 * * *`), not a user; auth is a `CRON_SECRET` bearer check (Vercel attaches that header itself for scheduled invocations) rather than `getVerifiedUser()`. For every plan where `isCheckInDue()` (`src/recommendations.js` — the same threshold the check-in button's enabled state uses) says a check-in is overdue and `plans.last_reminder_sent_at` isn't within the last `CHECKIN_INTERVAL_DAYS`, emails the plan owner via Resend and stamps `last_reminder_sent_at`. Skips anyone with `profiles.email_reminders_enabled = false`.
- `unsubscribe-reminders.js` — the plain-text link in that email's footer; flips `email_reminders_enabled` to false for one user. No session (email links can't carry auth headers), so `api/_lib/reminderToken.js` HMACs the userId (using `SUPABASE_SERVICE_ROLE_KEY` as the secret) to prove the click is for that user's own account.

### Data model (Supabase)
A `profiles` table (`has_paid`, `generation_credits`, `plans_generated`, `free_action_used`, `email_reminders_enabled`) drives all access control — always read via a service-role client keyed off the verified user id, never client-supplied. `supabase/migrations/*.sql` are plain SQL files with no migration tooling wired up — apply manually in the Supabase SQL editor (see comment in `0001_add_free_action_used.sql`). `checkins.is_deload` (`0002_add_checkin_is_deload.sql`) records whether the week a check-in reports on was a scheduled deload week. `plans.last_reminder_sent_at` (`0004_add_checkin_reminders.sql`) is the only writer/reader for the reminder cron's once-per-overdue-period gate. Unpaid users' in-progress plans are cached client-side in `localStorage` (`fitplan_pending_plan_<userId>`, 24h TTL) until they pay to save server-side; the not-yet-submitted plan/grade form itself is separately cached (`fitplan_draft_form_<userId>`, same 24h TTL) so it survives a checkout redirect or refresh.
...
`computeStreak` (`src/recommendations.js`) is adherence-based, not a raw check-in count: a check-in only "counts" toward the streak if >= 70% of that week's scheduled exercises were marked done (`computeWeekCompletionRate`); a below-70% week breaks the streak the same way a >10-day gap between check-ins does, and either way the next qualifying check-in restarts at 1. `STREAK_BADGES` (3/6/10 weeks, in `constants.js`) is unaware of this distinction — it just compares the streak number the caller passes in, so it applies unchanged. Lifetime completed-exercises (`computeLifetimeCompleted`) is a separate, unrelated stat: no plan_id filter, no gap/threshold logic, sums every exercise ever marked done across every plan for the user, sourced from a `checkins` query scoped by `user_id` alone (`loadLifetimeCheckins`, session-scoped, not tied to the currently loaded plan the way `checkins` state is). Only the streak (not the lifetime total) is ever sent to `ADJUST_SYSTEM_PROMPT`.

### Environment variables
Frontend (`VITE_` prefixed, safe to expose): `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`.
Server-only (used by `api/*.js`, never exposed to the client): `ANTHROPIC_API_KEY`, `RAPIDAPI_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `RESEND_API_KEY`, `CRON_SECRET` (protects `send-checkin-reminders.js` — Vercel sets this header itself for scheduled invocations once the var exists). `RESEND_FROM_EMAIL` and `APP_URL` are optional, both fall back to a working default (Resend's shared test sender, and the production `vercel.app` domain) if unset.
