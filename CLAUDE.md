...
The "▶ how to" button opens a video in an in-app modal via a plain, unauthenticated `youtube.com/embed/{id}` iframe — no API key or network call involved. `src/exerciseVideos.js` is a hand-curated `{ "Exercise Name": "youtubeVideoId" }` map (keys must match the clean base names the AI is instructed to produce — see EXERCISE NAMING below); any exercise name missing from it falls back to opening a plain YouTube search in a new tab instead of the modal (`openVideoModal` in `App.jsx`).
