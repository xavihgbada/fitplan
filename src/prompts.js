...
SESSION BALANCE — CRITICAL: in a full-body or upper/lower session (the default per SPLIT STRUCTURE below at 2-3 gym days/week), never let more than 2 of the exercises target the same primary muscle group — these sessions should train multiple muscle groups in a balanced, close-to-full-body way rather than concentrating on one area. In a body-part-split session (4+ gym days/week) built around specific focus muscles for that day (e.g. a chest/shoulders/triceps push day), that day's own focus muscle group(s) are exempt from this cap — concentrating volume on the focus muscle within its dedicated session is the entire point of a split day, and the VOLUME GUIDELINES above still bound how far it can go. Any other, non-focus muscle group that shows up incidentally in that session still follows the 2-exercise cap. Check your own exercise list against this rule before finalizing the plan.
...
- NEVER give two exercises in the same day's workout the identical name, even as different variations (e.g. two "Cable Lateral Raise" entries) — this collides with per-exercise progress tracking, which is keyed by day + name. If a session genuinely calls for the same movement pattern twice, that's the signal it belongs on separate days or should be a different exercise entirely, not a repeated name within one day.
...
- SESSION BALANCE — CRITICAL: never let more than 2 exercises in a session target a primary muscle group that isn't one of that session's established focus muscles, per the original plan's own split structure (e.g. don't add a 3rd shoulder exercise to a leg day). A session's own focus muscle group(s), as already established by the original plan (e.g. a dedicated push day's chest/shoulders/triceps focus), are exempt from this cap — the VOLUME GUIDELINES above still bound how far it can go. This matters most for full-body/upper-lower low-frequency plans (2-3 days/week), where no single muscle should dominate a session.
